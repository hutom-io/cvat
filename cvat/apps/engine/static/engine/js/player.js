...
class PlayerModel extends Listener {
    ...
    get geometry() {
        ...
        copy.rotation = this._settings.rotateAll
            ? this._geometry.rotation
    ...
    onFrameLoad(last) {
        // callback for FrameProvider instance
        if (last === this._frame.current) {
            ...
            if (this._continueAfterLoad) {
                ...
                // Just update frame if no need to play
                this.shift(0)
            }
        }
    }
    ...
    play() {
        ...
        this._playInterval = setInterval(() => {
            if (this._pauseFlag) {
                // pause method without notify (for frame downloading)
    ...
    shift(delta, absolute) {
        ...
        if (
            this._settings.resetZoom ||
            this._frame.previous === null ||
            differentRotation
        ) {
    ...
    fit() {
        ...
        this._geometry.top =
            (this._geometry.height - img.height * this._geometry.scale) / 2
        this._geometry.left =
    ...
    scale(point, value) {
        ...
        this._geometry.left +=
            point.x *
            (oldScale / this._geometry.scale - 1) *
            this._geometry.scale
        this._geometry.top +=
            point.y *
            (oldScale / this._geometry.scale - 1) *
    ...
    rotate(angle) {
        ...
        if (this._settings.rotateAll) {
            ...
            typeof this._framewiseRotation[this._frame.current] === "undefined"
        ) {
...
class PlayerController {
    constructor(playerModel, activeTrack, find, playerOffset) {
        ...
        function setupPlayerShortcuts(playerModel) {
            ...
            Mousetrap.bind(
                shortkeys.next_filter_frame.value,
                nextFilterFrameHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.prev_filter_frame.value,
                prevFilterFrameHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.next_key_frame.value,
                nextKeyFrameHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.prev_key_frame.value,
                prevKeyFrameHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.forward_frame.value,
                forwardHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.backward_frame.value,
                backwardHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.play_pause.value,
                playPauseHandler,
                "keydown"
            )
            Mousetrap.bind(
                shortkeys.clockwise_rotation.value,
                e => {
                    e.preventDefault()
                    this.rotate(90)
                },
            ...
            Mousetrap.bind(
                shortkeys.counter_clockwise_rotation.value,
                e => {
                    e.preventDefault()
                    this.rotate(-90)
                },
    ...
    zoom(e, canvas) {
        const point = window.cvat.translate.point.clientToCanvas(
            canvas,
            e.clientX,
            e.clientY
        )

        const zoomImageEvent = Logger.addContinuedEvent(
            Logger.EventType.zoomImage
        )
    ...
    frameMouseMove(e) {
        if (this._moving) {
            if (!this._events.move) {
                this._events.move = Logger.addContinuedEvent(
                    Logger.EventType.moveImage
                )
            }
    ...
    _rewind(e) {
        if (this._rewinding) {
            if (!this._events.jump) {
                this._events.jump = Logger.addContinuedEvent(
                    Logger.EventType.jumpFrame
                )
            }
            ...
            const targetFrame = Math.round(
                (frames.stop - frames.start) * percent
            )
            this._model.pause()
...
class PlayerView {
    constructor(playerModel, playerController) {
        ...
        this._playerContentUI.on("mousedown", e => {
            const pos = window.cvat.translate.point.clientToCanvas(
                this._playerBackgroundUI[0],
                e.clientX,
                e.clientY
            )
        ...
        this._playerContentUI.on("wheel", e =>
            this._controller.zoom(e, this._playerBackgroundUI[0])
        )
        this._playerContentUI.on("dblclick", () => this._controller.fit())
        this._playerContentUI.on("mousemove", e =>
            this._controller.frameMouseMove(e)
        )
        this._progressUI.on("mousedown", e =>
            this._controller.progressMouseDown(e)
        )
        this._progressUI.on("mouseup", () => this._controller.progressMouseUp())
        this._progressUI.on("mousemove", e =>
            this._controller.progressMouseMove(e)
        )
        ...
        this._multiplePrevButtonUI.on("click", () =>
        ...
        Mousetrap.bind(
            shortkeys.focus_to_frame.value,
            () => this._frameNumber.focus(),
            "keydown"
        )
        Mousetrap.bind(
            shortkeys.change_grid_opacity.value,
        ...
        Mousetrap.bind(
            shortkeys.change_grid_color.value,
            Logger.shortkeyLogDecorator(() => {
                ...
                const idx =
                    colors.indexOf(this._playerGridPath.attr("stroke")) + 1
        ...
        this._progressUI["0"].max =
            playerModel.frames.stop - playerModel.frames.start
        ...
        this._playerUI.on("contextmenu.playerContextMenu", e => {
            if (!window.cvat.mode) {
                ...
                const x = Math.min(
                    e.pageX,
                    this._playerUI[0].offsetWidth -
                        this._contextMenuUI[0].scrollWidth
                )
                const y = Math.min(
                    e.pageY,
                    this._playerUI[0].offsetHeight -
    ...
    onPlayerUpdate(model) {
        ...
        if (
            this._playerBackgroundUI.css("background-image").slice(5, -2) !==
            image.src
        ) {
            this._playerBackgroundUI.css(
                "background-image",
                `url("${image.src}")`
            )
        }
        ...
        for (const obj of [this._playerContentUI, this._playerTextUI]) {
            ...
            obj.css(
                "left",
                geometry.left - geometry.frameOffset * geometry.scale
            )
        }

        this._playerContentUI.css("transform", `scale(${geometry.scale})`)
        this._playerTextUI.css(
            "transform",
            `scale(10) rotate(${-geometry.rotation}deg)`
        )
